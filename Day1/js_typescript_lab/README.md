...
Before working through the exercises, get your project scaffolded and running.

1. Clone this repo and `cd` into it.
2. Run `npm create vite@latest es6-lab -- --template vanilla-ts` to scaffold the project.
3. `cd` into `es6-lab`.
...
// Your code here
newNums = nums.prototype.map((thisNum) => {return 2 * thisNum;});
console.log(newNums)
```
...
const propertyName = "myName1";
// Use propertyName as a dynamic key in userProfile, assigning a relevant value.
userProfile[propertyName] = "Bob";
// Your code here
```
...
// Your code here
function cat_sentence(noun='cat',adjective='white') {
    console.log(`The ${} was ${adjective}.`);
};
```
...
Assume we have a variable called `localLangConfig` that might contain a language code, like 'es' for Spanish or 'fr' for French, or might be `null` if no language is selected. Your task is to create a variable `LANG` that uses the value from `localLangConfig` if it's not null. If `localLangConfig` is null, default `LANG` to 'en' for English.
...
Now, let's try this same pattern for setting a user's website theme.
